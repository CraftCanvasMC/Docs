...
`folia-supported`/`canvas-supported` flag(s). To make your plugin compatible, the plugin developer must add support themselves.
We aim to abide completely by Folias new rules, and we will not remove thread checks, alter safety guards,
or blatantly remove systems that keep Canvas and Folia stable.
...
:::danger[Incompatible Java Version]
