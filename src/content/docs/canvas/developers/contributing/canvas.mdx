...
change it to `PaperServer`, for Minecraft classes, change it to `Minecraft` and so on.info
:::
...
When it happens, you have to manually resolve those conflicts using Git, following the instructions provided by the hints, and finishing off with a rebuild.Patches

### Creating new base patches

You can create base patches in 2 ways.
The first way (recommended), is to do it manually, using Git.patches
